...
Clock 保留上游 v1.6.5 天气时钟的页面、传感器、RTC、音频、配网和天气能力（包括聚合时钟页以及状态读取/LVGL 锁恢复修复）。其设置菜单的“更换 Wi-Fi”仅请求临时 AP+STA 配网门户；保存新凭据后门户退出。项目当前没有常驻局域网 Web 控制台。

上游的单镜像 OTA 在本项目中被禁用：它默认选择下一个 OTA 槽位，而这里的 `ota_1` 是 Display。更新必须使用匹配的六文件完整刷写包，以保持两种应用的协议与分区一致。
