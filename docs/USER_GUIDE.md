...
默认工作页包括：天气时钟、图片时钟、天气看板、温湿时钟、日历、温湿历史、小智 AI 和聚合时钟页。可在 `设置 → 显示` 中开关和排序页面，系统始终保留至少一个页面。
...
本项目的“检查更新”不会安装上游单镜像 OTA，因为该路径会占用 `ota_1` 并破坏 Display。需要更新时请下载本仓库一整套 `release/` 文件，执行完整刷写；该流程会保留 NVS 中的 Wi-Fi 和用户设置。
