...
本仓库有意跟踪当前已验证的 `release/` 六文件镜像，以及 `windows/driver/` 中的 Windows x64 驱动源码、可复核载荷和紧凑安装器，方便新用户完整刷写与安装。构建缓存、私密设备配置和本机驱动签名材料不进入仓库。
...
| 发布版本的校验值、`windows/driver/` 的源码/载荷/安装器 | 本机测试签名证书、私有签名密钥、构建缓存、私人截图 |
...
Release Notes 至少列出：硬件型号、ESP-IDF 版本、六个镜像的 SHA-256、Clock/Display 的 KEY 长按切换方式、`windows/driver/` 的 x64 测试签名安装前提、2.4 GHz Wi-Fi 限制、已知 RLCD 画面边界，以及完整刷写回退步骤。
