...
// mode.ini is shared with the Mono1 encoder.  Direct file reads avoid the
// cached values returned by GetPrivateProfileIntW inside a UMDF host.
static int rlcd_runtime_fps(int fallback)
{
	static ULONGLONG next_read = 0;
	static int configured_fps = 60;
	...
	if (now >= next_read) {
		...
		if (file != INVALID_HANDLE_VALUE) {
			...
			CloseHandle(file);
			const char* value = strstr(text, "Fps=");
			if (value) {
				int parsed = 0;
				for (const char* p = value + 4; *p >= '0' && *p <= '9'; ++p) parsed = parsed * 10 + (*p - '0');
				if (parsed >= 10 && parsed <= 60) configured_fps = parsed;
			}
		}
		next_read = now + 500;
	}
	return configured_fps >= 10 && configured_fps <= 60 ? configured_fps : fallback;
}
    ...
    for(;;) {
        ...
        if(hr == E_PENDING) {
            ...
            //LOG("-dxgi fid:%d dirty:%d\n",Buffer.MetaData.PresentationFrameNumber,Buffer.MetaData.DirtyRectCount);
            {
				...
				int effective_fps = rlcd_runtime_fps(pContext->fps);
				long fps_interval = 1000000L / effective_fps - 1000L;
				if (fps_interval < 1000L) fps_interval = 1000L;
				LOGD("last:%lld now:%lld diff:%lld intv%ld\n",last_fps_us,swap_t,swap_t-last_fps_us,fps_interval);
				if((swap_t-last_fps_us)>= fps_interval) // issue at the selected 10-60 FPS rate
				{
					...
					if(NULL != purb)
					{
						...
						c=get_system_us();
						uint8_t *mono = &purb->urb_msg[sizeof(udisp_frame_header_t)];
						if (total_bytes == 15000 && last_mono_frame_valid && memcmp(last_mono_frame, mono, 15000) == 0) {
							InterlockedPushEntrySList(&urb_list, &(purb->node));
							goto next;
						}
						if (total_bytes == 15000) { memcpy(last_mono_frame, mono, 15000); last_mono_frame_valid = true; }
	...
	*blimit=1920*1080*4;//1080P full
	...
	for(i=1;i<cnt;i++){
		...
		case 'E':
			...
			switch(str[1]){
			case 'm':
				if (strncmp(str, "Emono1", 6) == 0) {
					*enc=UDISP_TYPE_MONO1;
					enc_cfged++;
				}
				break;
			case 'j':
