...
static DRAM_ATTR  const uint8_t Zig[64] = {	/* Zigzag-order to raster-order conversion table */
...
static DRAM_ATTR  const uint16_t Ipsf[64] = {	/* See also aa_idct.png */
...
static DRAM_ATTR  const uint8_t Clip8[1024] = {
    /* 0..255 */
...
static IRAM_ATTR OPTIMIZE_ATTR uint8_t BYTECLIP(int val)
{
    if(val < 0) return 0;
...
static IRAM_ATTR OPTIMIZE_ATTR void* alloc_pool(  /* Pointer to allocated memory block (NULL:no memory available) */
...
static IRAM_ATTR OPTIMIZE_ATTR JRESULT create_qt_tbl(  /* 0:OK, !0:Failed */
    JDEC* jd,				/* Pointer to the decompressor object */
...
)
{
    ...
    while(ndata) {	/* Process all tables in the segment */
        ...
        if(!pb) {
            ESP_LOGE(__FUNCTION__, "alloc qttbl %d NG", 64 * sizeof(int32_t));
...
static IRAM_ATTR OPTIMIZE_ATTR JRESULT create_huffman_tbl(  /* 0:OK, !0:Failed */
    JDEC* jd,					/* Pointer to the decompressor object */
...
)
{
    ...
    while(ndata) {	/* Process all tables in the segment */
        ...
        if(!pb) {
            ESP_LOGE(__FUNCTION__, "alloc huf %d NG", 16);
            return JDR_MEM1;            /* Err: not enough memory */
        }
        ...
        if(!ph)  {
            ESP_LOGE(__FUNCTION__, "alloc huf %d NG", np * sizeof(uint16_t));
            return JDR_MEM1;           /* Err: not enough memory */
        }
        ...
        if(!pd) {
            ESP_LOGE(__FUNCTION__, "alloc huf2 %d NG", np);
            return JDR_MEM1;            /* Err: not enough memory */
        }
        ...
        {	/* Create fast huffman decode table */
            ...
            if(cls) {
                ...
                if(!tbl_ac) {
                    ESP_LOGE(__FUNCTION__, "alloc huf3 %d NG", HUFF_LEN * sizeof(uint16_t));
                    return JDR_MEM1;        /* Err: not enough memory */
                }
                ...
                if(!tbl_dc) {
                    ESP_LOGE(__FUNCTION__, "alloc huf4 %d NG", HUFF_LEN * sizeof(uint8_t));
                    return JDR_MEM1;        /* Err: not enough memory */
                }
...
static IRAM_ATTR OPTIMIZE_ATTR int huffext(  /* >=0: decoded data, <0: error code */
    JDEC* jd,			/* Pointer to the decompressor object */
...
static IRAM_ATTR OPTIMIZE_ATTR int bitext(  /* >=0: extracted data, <0: error code */
    JDEC* jd,			/* Pointer to the decompressor object */
...
static IRAM_ATTR OPTIMIZE_ATTR JRESULT restart(
    JDEC* jd,		/* Pointer to the decompressor object */
...
static IRAM_ATTR OPTIMIZE_ATTR void block_idct(
...
static IRAM_ATTR OPTIMIZE_ATTR JRESULT mcu_load(
    JDEC* jd		/* Pointer to the decompressor object */
)
{
...
static IRAM_ATTR OPTIMIZE_ATTR JRESULT mcu_output(
    JDEC* jd,			/* Pointer to the decompressor object */
...
JRESULT IRAM_ATTR OPTIMIZE_ATTR jd_prepare(
...
)
{
    ...
    if(!seg) {
        ESP_LOGE(__FUNCTION__, "alloc inbuf %d NG", JD_SZBUF);
        return JDR_MEM1;
    }
    ...
    do {
        if(jd->infunc(jd, seg, 1) != 1) {
            ESP_LOGE(__FUNCTION__, "infunc can't read 2 byte for SOI");
    ...
    for(;;) {				/* Parse JPEG segments */
        ...
        if(jd->infunc(jd, seg, 4) != 4) {
            ESP_LOGE(__FUNCTION__, "infunc can't read 4 byte for Maker");
            return JDR_INP;
        }
        ...
        case 0xC0:	/* SOF0 (baseline JPEG) */
            ...
            if(jd->infunc(jd, seg, len) != len) {
                ESP_LOGE(__FUNCTION__, "infunc can't read %d byte for 0xc0", len);
        ...
        case 0xDD:	/* DRI - Define Restart Interval */
            if(len > JD_SZBUF) return JDR_MEM2;
            if(jd->infunc(jd, seg, len) != len) {
                ESP_LOGE(__FUNCTION__, "infunc can't read %d byte for 0xdd", len);
                return JDR_INP; /* Load segment data */
            }
        ...
        case 0xC4:	/* DHT - Define Huffman Tables */
            if(len > JD_SZBUF) return JDR_MEM2;
            if(jd->infunc(jd, seg, len) != len) {
                ESP_LOGE(__FUNCTION__, "infunc can't read %d byte for 0xc4", len);
                return JDR_INP; /* Load segment data */
            }
        ...
        case 0xDB:	/* DQT - Define Quaitizer Tables */
            if(len > JD_SZBUF) return JDR_MEM2;
            if(jd->infunc(jd, seg, len) != len) {
                ESP_LOGE(__FUNCTION__, "infunc can't read %d byte for 0xdb", len);
                return JDR_INP; /* Load segment data */
            }
        ...
        case 0xDA:	/* SOS - Start of Scan */
            if(len > JD_SZBUF) return JDR_MEM2;
            if(jd->infunc(jd, seg, len) != len) {
                ESP_LOGE(__FUNCTION__, "infunc can't read %d byte for 0xda", len);
                return JDR_INP; /* Load segment data */
            }
            ...
            if(!jd->workbuf) {
                ...
                ESP_LOGE(__FUNCTION__, "alloc workbuf %d NG", len);
                return JDR_MEM1;            /* Err: not enough memory */
            }
            jd->mcubuf = alloc_pool(jd, (n + 2) * 64 * sizeof(jd_yuv_t));	/* Allocate MCU working buffer */
            if(!jd->mcubuf) {
                ESP_LOGE(__FUNCTION__, "alloc mcubuf %d NG", (n + 2) * 64 * sizeof(jd_yuv_t));
                return JDR_MEM1;            /* Err: not enough memory */
            }
        ...
        default:	/* Unknown segment (comment, exif or etc..) */
            ...
            if(jd->infunc(jd, 0, len) != len) {
                ESP_LOGE(__FUNCTION__, "infunc can't read %d byte for default", len);
                return JDR_INP;
            }
        }
    }
...
JRESULT IRAM_ATTR OPTIMIZE_ATTR jd_decomp(
